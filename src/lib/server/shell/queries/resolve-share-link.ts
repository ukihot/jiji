...
export async function resolveShareLinkByToken(
	...
	return { link, isActive: isShareLinkActive(link, now), claimedPersonName: claimedPerson?.name ?? null };
}

/** design.md 8.4節のhooks.server.ts用: Cookieのshare_tokenから「今の自分」を引く */
export async function resolveClaimedPersonByToken(db: SqliteDb, token: string): Promise<PersonRow | null> {
	const link = await getShareLinkByTokenHash(db, hashShareToken(token));
