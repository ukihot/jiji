...
<section class="border-border bg-surface mt-6 rounded-xl border p-3" aria-label="Timeline">
	...
	{:else}
		<SequenceBar
			cuts={data.view.cuts}
			totalFrames={data.view.totalFrames}
			cutHref={(cutId) => `/${data.view.title.id}/${data.view.timeline.id}/cuts/${cutId}`}
		/>
	{/if}
</section>
				...
				<div class="border-border border-t p-4">
					<form method="POST" action="?/addCut" class="flex flex-wrap items-end gap-2">
						...
						<FormField label={m.label_scene_tags()}>
							<FormInput type="text" name="sceneTags" placeholder={m.placeholder_scene_tags()} />
						</FormField>
