...
{:else if needsName}
	...
	{#if form?.message}
		<p class="text-danger mt-3 flex items-center gap-1.5 text-sm">
			<AlertTriangle size={16} aria-hidden="true" />
			{form.message}
		</p>
	{/if}
...
{:else}
	...
	<dl class="text-muted mt-3 space-y-1.5 text-sm">
		<div class="flex items-center gap-1.5">
			...
			<dd class="border-border rounded-full border px-2 py-0.5 text-xs">
				{data.resolved.link.permissionLevel}
			</dd>
		</div>
		<div class="flex items-center gap-1.5">
	...
	<p class="text-muted mt-4 text-xs">
		提出・レビュー機能（Submission/Review）はdesign.md 13章
		Phase3で実装予定のため、このMVPではまだ利用できません。
	</p>
{/if}
